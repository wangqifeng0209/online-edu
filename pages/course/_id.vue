                  ...
                  <section class="mt20">
                    <div class="lh-menu-wrap">
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          <!-- 文件目录 -->
                          <li class="lh-menu-stair" v-for="(chapter,index) in chapterVideo" :key="index">
                            <a href="javascript: void(0)" :title="chapter.title" class="current-1">
                              <em class="lh-menu-i-1 icon18 mr10"></em>{{chapter.title}}
                            </a>
                            <ol class="lh-menu-ol" style="display: block;">
                              <li class="lh-menu-second ml30" v-for="video in chapter.children" :key="video.id">
                                <a target="_blank" :href="'/player/' + video.videoSourceId" title>
                                  <span class="fr">
  ...
  export default {
    ...
    asyncData({params, error}){
      return course.getCourseInfo(params.id)
        .then(response => {
          return {course: response.data.data.courseInfo, chapterVideo: response.data.data.chapter}
        })
    }
  }
</script>
